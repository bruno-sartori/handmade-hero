/*
  ...
  - Make the right calls so windows doesnt think we are still loading for a bit after we actually start
...
internal void Win32DisplayBufferInWindow(win32_offscreen_buffer *Buffer, HDC DeviceContext, int WindowWidth, int WindowHeight) {
  // IMPORTANT: Este calculo apenas funciona para monitores 1920x1080, o meu monitor primario 1366x768 nao funciona, mudar if
...
int CALLBACK WinMain(HINSTANCE Instance, HINSTANCE PrevInstance, LPSTR CommandLine, int ShowCode) {
  ...
  if (RegisterClassA(&WindowClass)) {
    HWND Window = CreateWindowExA(0 /* WS_EX_TOPMOST|WS_EX_LAYERED*/, WindowClass.lpszClassName, "Handmade Hero", WS_OVERLAPPEDWINDOW | WS_VISIBLE, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, 0, 0, Instance, 0);
    ...
    if (Window) {
      ...
      // TODO: TransientStorage needs to be broken up into game transient and cache
      // transient, and only the former need be saved for state playback.
      ...
      if (Samples && GameMemory.PermanentStorage && GameMemory.TransientStorage) {
        ...
        while (GlobalRunning) {
          ...
          NewInput->ExecutableReload = false;
          ...
          if (CompareFileTime(&NewDLLWriteTime, &Game.DLLLastWriteTime) != 0) {
            ...
            NewInput->ExecutableReload = true;
          }
