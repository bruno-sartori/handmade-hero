...
inline v2 GetRenderEntityBasisP(render_group *RenderGroup, render_entity_basis *EntityBasis, v2 ScreenCenter) {
  ...
  real32 ZFudge = 1.0f + 0.1f * EntityBaseP.z; // scaling
  ...
  for(uint32 BaseAddress = 0; BaseAddress < RenderGroup->PushBufferSize;) {
    ...
    switch (Header->Type) {
      ...
      case RenderGroupEntryType_render_entry_coordinate_system: {
        ...
        DrawRectangleSlowly(OutputTarget, Entry->Origin, Entry->XAxis, Entry->YAxis, Entry->Color, Entry->Texture, Entry->NormalMap, Entry->Top, Entry->Middle, Entry->Bottom, 1.0f / RenderGroup->MetersToPixels);
