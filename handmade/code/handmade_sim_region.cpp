...
internal bool32 SpecullativeCollide(sim_entity *Mover, sim_entity *Region, v3 TestP) {
  bool32 Result = true;
  if (Region->Type == EntityType_Stairwell) {
    // TODO: Needs work :)
...
internal void MoveEntity(game_state *GameState, sim_region *SimRegion, sim_entity *Entity, real32 dt, move_spec *MoveSpec, v3 ddP) {
  ...
  for (uint32 Iteration = 0; Iteration < 4; ++Iteration) {
    ...
    if (PlayerDeltaLength > 0.0f) {
      ...
      v3 WallNormalMin = {}; // r -> normal vector of the surface (new Y coordinate relative to wall (new x))
      v3 WallNormalMax = {};
                ...
                if (Rel.z >= MinCorner.z && Rel.z < MaxCorner.z) {
                  ...
                  if (IsSet(TestEntity, EntityFlag_Traversable)) {
                    ...
                    if (HitThis) {
                      ...
                      if (SpecullativeCollide(Entity, TestEntity, TestP)) {
                        tMin = tMinTest;
